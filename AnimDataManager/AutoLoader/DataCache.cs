...
namespace AnimDataManager.AutoLoader
{
    public sealed class DataCache
    {
        ...
        private ConcurrentDictionary<Type, CacheWrapperBase> cacheData = new ConcurrentDictionary<Type, CacheWrapperBase>();
        ...
        where T2 : DtoBase<T2>, new()
        {
            CacheWrapperBase cacheWrapperBase = new DataCacheWrapper<T1, T2>();
            return cacheData.TryRemove(typeof(T1), out cacheWrapperBase);
        }

        public DataCacheWrapper<T1, T2> GetDataCacheWrapper<T1, T2>()
        where T1 : DaoBase<T2>, new()
        where T2 : DtoBase<T2>, new()
        {
            return (DataCacheWrapper<T1, T2>)cacheData[typeof(T1)];
        }

        public void ClearAll()
        {
            foreach(CacheWrapperBase dataCacheWrapper in Instance.cacheData.Values)
            {
